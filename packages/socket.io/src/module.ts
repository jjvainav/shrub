...
export interface ISocketIOConfiguration extends Server {
}

export const ISocketIOServer = createService<ISocketIOServer>("socket.io-server");
export interface ISocketIOServer extends Server {
}
...
export class SocketIOModule implements IModule {
    ...
    configureServices(registration: IServiceRegistration): void {
        registration.registerSingleton(ISocketIOServer, { create: services => new Server(services.get(IHttpServer)) });
    }
}