<template>
    <div id="container">
        <section class="todoapp">
            <header class="header">
                <!-- Somehow the header needs a margin of 83 but it's not get set properly so it is hardcoded for now -->
                <h1 style="margin: 83px 0;">todos</h1>
                <input
                    class="new-todo"
                    autofocus
                    autocomplete="off"
            ...
            <section class="main" v-show="model.hasItems()" v-cloak>
                <input id="toggle-all" class="toggle-all" type="checkbox" v-model="model.allCompleted" />
                <label for="toggle-all"></label>
                <ul class="todo-list">
                    <li
                        v-for="item in model.getFilteredItems()"
                        class="todo"
                        :key="item.id"
                        ...
                        <input
                            ...
                            type="text"
                            v-model="item.title"
                            v-todo-focus="model.isBeingEdited(item)"
                            @blur="model.endEdit()"
                            @keyup.enter="model.endEdit()"
                            @keyup.esc="model.cancelEdit()"/>
                    </li>
                </ul>
            </section>
...
<script lang="ts" src="./todo"></script>
...
<style scoped>
    ...
    /* 
     * The below css is based on https://unpkg.com/todomvc-app-css@2.2.0/index.css
     * but is not identical, there have been a few tweaks to work properly inside the workbench
    */
